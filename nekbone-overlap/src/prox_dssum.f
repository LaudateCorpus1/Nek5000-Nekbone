...
c     call nekgsync()
#ifdef HPE_OVERLAP_MOD
      call gs_op(gsh,f,1,1,0,c1)   ! Gather-scatter operation  ! w   = QQ  w
#else
...
#ifdef HPE_OVERLAP_MOD
c-----------------------------------------------------------------------
      subroutine refactor_setupds(gs_handle,nx1,ny1,nz1,mx,my,mz,mxsz)

      integer gs_handle,nx1,ny1,nz1,mx,my,mz,nelt,ntot,mxsz
   
c      t0 = dnekclock()
      nelt      = mx*my*mz
      ntot      = nx1*ny1*nz1*nelt
      call rf_setup(gs_handle,nx1,ny1,nz1,mx,my,mz,ntot,nelt,mxsz)
      
c      t1 = dnekclock() - t0
c      if (nid.eq.0) then
c         write(6,1) t1,rf_handle,nx1,dof
c    1    format(' RF setupds time',1pe11.4,' seconds ',2i3,i12)
c      endif
