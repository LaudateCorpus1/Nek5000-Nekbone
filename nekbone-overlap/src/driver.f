c-----------------------------------------------------------------------
     ...
     $    ,zpts(2*lx1),wght(2*lx1)
      ...
      real, target, dimension((2*lt)) :: wp_joined
      real, dimension(:), pointer :: w,p
      real x(lt),f(lt),r(lt),z(lt),c(lt)
#else
...
#ifdef HPE_OVERLAP_MOD
           call refactor_setupds (gsh,nx1,ny1,nz1,mx,my,mz,lt)
#endif
...
c-----------------------------------------------------------------------
      ...
      call rone(c,n)
#ifdef HPE_OVERLAP_MOD
      call gs_op(gsh,c,1,1,0,c1) ! Gather-scatter operation  ! w   = QQ  w
#else
