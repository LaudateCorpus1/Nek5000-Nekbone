  ...
  unsigned transpose, const void *execdata, const struct comm *comm, char *buf)
{
   ...
   * Hence, the local variable 'p'. This local variable will be used
   * for accessing window for one-sided calls as well.
   */
  ...
  if (!thd) {
    ...
    #pragma omp master
      ...
      {
        /*
         * RMA_PT:Win_lock_all can be issued just after window creation
         */
        MPI_Win_lock_all(0, p->win);
      }
  ...
  if (inp) {
  ...
  #endif
    {
      ...
      /*
       * RMA_PT: Win_flush call doesn't guarantees the completion of data
       * transfer, using MPI_Barrier would ensure that data transfer is
       * completed and data is available for gather-from-buf call.
       */
      comm_barrier(comm);
  #else
...
static void pw_free(struct pw_data *data, const struct comm *comm)
{
#ifdef HPE_ONE_SIDED
  /*
