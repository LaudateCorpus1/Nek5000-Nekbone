#ifdef HPE_ONE_SIDED
  /*
   * We need the PUT or GET macro to be defined 
   * If either is not defined, define HPE_USING_GET
   */
  #ifndef HPE_USING_PUT
    #ifndef HPE_USING_GET
      #define HPE_USING_GET
    #endif
  #endif
#endif
...
struct pw_comm_data {
  ...
  size_t *offsets;
#ifdef HPE_ONE_SIDED
  size_t *r_offsets; /* HPE: offsets seen at remote processor */
#endif
};
...
#ifdef HPE_ONE_SIDED
  buffer rma_buf;
  MPI_Win win;       /* HPE: MPI window */
  MPI_Group group_target, group_origin; /*HPE: PSCW: MPI processor group for target and origin ranks */
#endif
};
...
#ifdef HPE_ONE_SIDED
static char *pw_exec_puts(char *buf, const unsigned unit_size,
                           const struct comm *comm,
                           const struct pw_comm_data *c_snd,
                           const struct pw_comm_data *c_rcv,
                           MPI_Win *win)
{
  const uint *targetRank    =c_snd->p,
             *size =c_snd->size;
  int i;
  char *retVal = buf;
  ...
  for(i = 0; i < c_snd->n; i++) {
    comm_put(win,
             comm,
             buf+c_snd->offsets[i]*unit_size,
             size[i]*unit_size,
             targetRank[i],
             c_rcv->r_offsets[i]*unit_size);
  }

  if (c_snd->n != 0) {
   retVal += c_snd->offsets[c_snd->n-1]*unit_size + size[c_snd->n-1]*unit_size;
  }

  return retVal;
}

static char *pw_exec_gets(char *buf, const unsigned unit_size,
                           const struct comm *comm,
                           const struct pw_comm_data *c_rcv,
                           const struct pw_comm_data *c_snd,
                           MPI_Win *win)
{
  const uint *sourceRank    =c_rcv->p,
             *size =c_rcv->size;
  int i;
  char *retVal = buf;
...
#pragma omp for
  ...
  for(i = 0; i < c_rcv->n; i++) {
    comm_get(win,
             comm,
             buf+c_rcv->offsets[i]*unit_size,
             size[i]*unit_size,
             sourceRank[i],
             c_snd->r_offsets[i]*unit_size);
  }

  if (c_rcv->n != 0) {
   retVal += c_rcv->offsets[c_rcv->n-1]*unit_size + size[c_rcv->n-1]*unit_size;
  }
  ...
  return retVal;
...
#ifdef HPE_ONE_SIDED
    /*
     * Illustration of buffer & RMA window usage in pw_exec()
     *
     * NOTE:
     * Receive and Send buffers are contiguous.
     * 'sendbuf' is the next consecutive address after the last byte of 'recvbuf'
     */

    #ifdef HPE_USING_PUT
    /*
     * MPI_Put() : Take data from local memory starting at 'sendbuf' and
     *             PUT it into RMA window on remote side (starting at recvbuf)
     * 
     *     |. . . . . . . . . . . . . . . Buffer . . . . . . . . . . . . . . .|
     *     |                                                                  |
     *     +--------------------------------++--------------------------------+
    ...
    /* 
     * 
     * MPI_Get() : GET data from RMA window on remote side (starting at 'sendbuf') and
     *             place it into local memory starting at 'recvbuf'
     * 
     *     |. . . . . . . . . . . . . . . Buffer . . . . . . . . . . . . . . .|
     *     |                                                                  |
     *     +--------------------------------++--------------------------------+
  ...
  unsigned transpose, const void *execdata, const struct comm *comm, char *buf)
{
  ...
  recvbuf_sz = 0;
  if (c->n != 0) {
    recvbuf_sz = c->offsets[c->n-1]*unit_size + c->size[c->n-1]*unit_size;
  }

  recvbuf = buf;
  sendbuf = (recvbuf + recvbuf_sz);
    ...
    sendbuf_sz = 0;
    if (c->n != 0) {
      sendbuf_sz = c->offsets[c->n-1]*unit_size + c->size[c->n-1]*unit_size;
    }
  ...
  /*
   * RMA window should be created only once per process.
   *
   * If 'MPITHREADS' is not defined, only the master thread may make MPI calls.
   * Hence, '#pragma omp single' cannot be used.
   */
  if (!thd) {
    /*
     * always master thread even if MPITHREADS is defined
     */

    if (pwd->win == MPI_WIN_NULL) {
      /*
       * The RMA window parameter to MPI_Win_create() needs to be modifiable.
       * Hence, the local variable 'p'.
       */
      struct pw_data *p = (struct pw_data *)execdata;
      ...
      MPI_Win_create(winbuf, winsize, sizeof(unsigned char), info, comm->c, &(p->win));
//VS! PSCW-2
  #ifndef MPITHREADS
    #pragma omp master
  #endif
      {
  #ifdef HPE_USING_PUT
    MPI_Win_post(pwd->group_target,0,pwd->win);
  #endif
      }
    }
    else {
    ...
    /*
     * Ensure all MPI processes synchronize at this point.
     */
//    comm_barrier(comm);
  }
#endif
  ...
  if (inp) {
  ...
  #endif
    {
        ...
        MPI_Win_post(pwd->group_target,0,pwd->win);
  #endif
        MPI_Win_start(pwd->group_origin,0,pwd->win);
      {
        struct pw_data *p = (struct pw_data *)execdata;
  #ifdef HPE_USING_PUT
        pw_exec_puts(sendbuf,unit_size,comm,&pwd->comm[send],&pwd->comm[recv],&(p->win));
  #else
        pw_exec_gets(recvbuf,unit_size,comm,&pwd->comm[recv],&pwd->comm[send],&(p->win));
  #endif
      }
        MPI_Win_complete(pwd->win);
  #ifdef HPE_USING_PUT
        MPI_Win_wait(pwd->win);      
  #endif
    }
...
#ifdef HPE_ONE_SIDED
  data->r_offsets = malloc(sizeof(size_t)*data->n);
#endif
}
...
#ifdef HPE_ONE_SIDED
  /*
   * This function is invoked from gs_free() within the OMP single-threaded
   * portion of 'src/driver.f:program nekbone'. Hence, no need to specify
   * single threading (#pragma omp master/single).
   *
   * NOTE:
   * data->win and  (data->rma_buf).ptr are allocated within the OpenMP
   * parallel portion in 'src/cg.f:subroutine cg()':
   * - data->win is allocated in pw_exec(), under '#pragma omp master'
   * - (data->rma_buf).ptr is allocated in gs_aux(), under '#pragma omp single'
   */

  if (data->win != MPI_WIN_NULL) {
  #ifdef HPE_USING_PUT
    MPI_Win_start(data->group_origin,0,data->win);
    MPI_Win_complete(data->win);
  #endif
    MPI_Win_wait(data->win);
    MPI_Win_free(&(data->win));
//VS! Freeing 
    MPI_Group_free(&(data->group_origin));
    MPI_Group_free(&(data->group_target));
//VS!
    data->win = MPI_WIN_NULL;
  }

  if ((data->rma_buf).ptr) {
    buffer_free(&(data->rma_buf));
    (data->rma_buf).ptr = NULL;
    (data->rma_buf).n   = 0;
    (data->rma_buf).max = 0;
  }
#endif
...
#if 0
  /*
   * HPE-comment: DEFECT Alert: This appears to be premature!!!
   */
  free(data);
#endif
...
#ifdef HPE_ONE_SIDED
  (pwd->rma_buf).ptr = NULL;
  (pwd->rma_buf).n   = 0;
  (pwd->rma_buf).max = 0;

  pwd->win = MPI_WIN_NULL;

  struct pw_comm_data *data;
  uint *p;

//VS! Moving group creation to setup phase: PSCW
  MPI_Group comm_group, group_target, group_origin;
  MPI_Comm_group(comm->c, &comm_group);
  const uint *proc_target    =(&pwd->comm[0])->p;
  const uint *proc_origin    =(&pwd->comm[1])->p;
  const uint num_target = (&pwd->comm[0])->n;
  const uint num_origin = (&pwd->comm[1])->n;
  MPI_Group_incl(comm_group, num_origin, proc_origin, &pwd->group_origin);
  MPI_Group_incl(comm_group, num_target, proc_target, &pwd->group_target);
//VS!
  ...
  for (j=0; j<2; j++){
    ...
    p=data->p;
    comm_req *req;
    req = tmalloc(comm_req,2*data->n);
    for (i = 0; i < data->n; i++) {
      comm_irecv(&(req[i]),comm,&data->r_offsets[i],sizeof(size_t),p[i],p[i]);
    }
    for (i = 0; i < data->n; i++) {
      comm_isend(&(req[i+data->n]),comm,&data->offsets[i],sizeof(size_t),p[i],comm->id);
    }
    comm_wait(req,2*data->n);
    free(req);
  }
#endif
}
...
//      printf("buffer_reserve: %d for rank %04d\n", vn*gs_dom_size[dom]*gsh->r.buffer_size, (int)comm_gbl_id);	//VS: print buffer size
      memset(buf->ptr, 0, vn*gs_dom_size[dom]*gsh->r.buffer_size);
	}
  }
#else
...
#ifdef HPE_ONE_SIDED
  /*
   * Communicators should only be freed up after one-sided cleanup
   * is completed. MPICH fails with a SEGV due to empty communicator
   * otherwise.
   */
  comm_free(&gsh->comm);
#endif
